...
(function ($) {
   ...
   =================================== */
    var SCROLL_DELAY = 350; // ms, koliko da sačekamo pre skrolovanja
    var SCROLL_DURATION = 700; // ms, trajanje animacije
    var EXTRA_OFFSET = 0; // px, mali buffer ispod header-a

    // hvata SVE <a> sa href="#..." (osim čistog "#"), i ne kida bootstrap-ove toggle linkove
    $(document).on('click', 'a[href^="#"]:not([href="#"]):not([data-bs-toggle]):not([data-toggle])', function (e) {
        var href = $(this).attr('href');
        var $target = $(href);
        if (!$target.length) return; // nema mete – pusti default

        e.preventDefault();

        // per-link override: <a data-scroll-delay="500">
        var perLinkDelay = parseInt($(this).attr('data-scroll-delay'), 10);
        var delay = isNaN(perLinkDelay) ? SCROLL_DELAY : perLinkDelay;

        var headerH = typeof getHeaderHeight === 'function' ? getHeaderHeight() : 0;
        var targetTop = Math.max(0, $target.offset().top - headerH - EXTRA_OFFSET);

        // zatvori mobilni meni ako je otvoren
        if ($('.navbar-collapse.show').length) $('.navbar-collapse').collapse('hide');

        // malo zakašnjenje pa glatko skrolovanje

        setTimeout(function () {
            $('html, body').stop().animate(
                { scrollTop: targetTop },
                SCROLL_DURATION,
                'swing' // default jQuery easing (radi bez dodatnih plugina)
            );
        }, delay);
    });
    ...
    $(document).on('click', 'body', function (e) {
        // Close all menu
        if (
            !($(e.target).closest('.navbar-nav').length || $(e.target).closest('.navbar-full-screen-menu-inner').length)
        ) {
            setTimeout(function () {
    ...
    function setTopSpaceHeight() {
        if (
            (!$('header').hasClass('sticky') && ($('.top-space-margin').length || $('.top-space-padding').length)) ||
    ...
    // ! Andrija - sva neophodna logika za Contact form validation je u /js/form.js fajlu. Ovde zakomentarisano.
    /* ===================================
        ...
        if ($('body').hasClass('custom-cursor')) {
            ...
            $('body').on('mouseenter', '.rounded-box', function () {
                ...
                cursorOuterEl.style.transform =
                    'translate(' + $elem.offset().left + 'px, ' + ($elem.offset().top - scrollTop) + 'px' + ')';
